            ...
            if first[0] != '!':
                ...
                if initial_mass != final_mass:
                    if re.search('mass\_change',line):
                        line = "\tmass_change = -1d-7  !set by driver.py"
...
tracks = ascii.read('coarse_grid_input_params_v1.txt', delimiter=',')
...
for track in tracks[idx]:
    ...
    for istage in range(len(stages)):
        ...
        if istage!=0:
            ...
            prev_stage = 'rgbtip'
            idx = np.where(single_star_tracks['initial_mass']==initial_mass)[0]
            if len(idx) > 0:
